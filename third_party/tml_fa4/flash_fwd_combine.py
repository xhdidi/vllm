...
from vllm.third_party.tml_fa4.seqlen_info import SeqlenInfo
from cutlass.cute import FastDivmodDivisor
import vllm.third_party.tml_fa4.pipeline as pipeline
...
class FlashAttentionForwardCombine:
    ...
    def __call__(
        ...
        mRowMax_partial: Optional[cute.Tensor] = None,
        ...
        mLSE_partial, mRowMax_partial = (
            cute.make_tensor(t.iterator, cute.select(t.layout, mode=LSE_partial_layout_transpose))
            if t is not None
            else None
            for t in (mLSE_partial, mRowMax_partial)
        )
        ...
        class SharedStorage:
            ...
            sRowMax: cute.struct.Align[
                cute.struct.MemRange[
                    Float32,
                    cute.cosize(self.smem_layout_lse) if const_expr(self.store_row_max) else 0,
                ],
                128,
            ]
    ...
    def kernel(
        ...
        mRowMax_partial: Optional[cute.Tensor],
        ...
        if const_expr(self.store_row_max):
            sRowMax = storage.sRowMax.get_tensor(smem_layout_lse)
        ...
        while work_tile.is_valid_tile:
            ...
            if const_expr(semaphore_to_reset is not None):
                if tidx == 0 and m_block == 0 and k_block == 0 and maybe_virtual_batch == 0:
            ...
            if (const_expr(num_splits_dynamic_ptr is None) or num_splits > 0) and (
                ...
                if const_expr(self.store_row_max):
                    if const_expr(cu_seqlens is None):
                        mRowMax_partial_cur = mRowMax_partial[None, None, None, batch_idx]
                        mRowMax_cur = mRowMax[None, None, batch_idx]
                    else:
                        mRowMax_partial_cur = cute.domain_offset((offset, 0, 0), mRowMax_partial)
                        mRowMax_cur = cute.domain_offset((offset, 0), mRowMax)
                    mRowMax_partial_copy = cute.tiled_divide(mRowMax_partial_cur, (1,))

                gmem_thr_copy_LSE = gmem_tiled_copy_LSE.get_slice(tidx)
                tLSEsLSE = gmem_thr_copy_LSE.partition_D(sLSE)

                if const_expr(self.store_row_max):
                    tg2ssRowMax = gmem_thr_copy_LSE.partition_D(sRowMax)
                ...
                for m in cutlass.range(cute.size(tLSEcLSE, mode=[2]), unroll_full=True):
                    ...
                    if idx < max_idx:
                        ...
                        if const_expr(self.store_row_max):
                            mRowMax_partial_cur_copy = mRowMax_partial_copy[
                            ...
                            if si < num_splits:
                                ...
                                if const_expr(self.store_row_max):
                                    if k_block == 0:
                                        cute.copy(
                                            gmem_thr_copy_LSE,
                                            mRowMax_partial_cur_copy[None, si],
                                            tg2ssRowMax[None, s, m],
                                        )
                            else:
                                tLSEsLSE[None, s, m].fill(-Float32.inf)
                                if const_expr(self.store_row_max):
                                    if k_block == 0:
                                        tg2ssRowMax[None, s, m].fill(-Float32.inf)
                ...
                # ===============================
                # Step 3B: Compute row max and store to GMEM
                # ===============================)

                if const_expr(self.store_row_max):
                    ts2rsRowMax = s2r_thr_copy_LSE.partition_S(sRowMax)
                    ts2rrRowMax = cute.make_fragment_like(ts2rsRowMax)

                    if k_block == 0:
                        cute.copy(s2r_tiled_copy_LSE, ts2rsRowMax, ts2rrRowMax)
                            ...
                            if ts2rcLSE[0, 0, m][0] == 0:
                                ...
                                if idx < max_idx:
                                    if const_expr(not varlen):
                                        ...
                                        head_idx = idx // seqlen
                                        m_idx = idx - head_idx * seqlen
                                    mRowMax_cur[m_idx, head_idx] = row_max
