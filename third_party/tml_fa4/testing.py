...
def generate_random_padding_mask(
    ...
    zero_lengths=False,
    min_seqlen=None,
    min_seqlen_array=None,
):
    assert mode in ["full", "random", "third"]
    assert min_seqlen_array is None or len(min_seqlen_array) == batch_size, (
        f"min_seqlen_array length {len(min_seqlen_array)} must match batch_size {batch_size}"
    )
    min_seqlen = min_seqlen if min_seqlen is not None else 0 if zero_lengths else 1
    if mode == "full":
        lengths = torch.full((batch_size, 1), max_seqlen, device=device, dtype=torch.int32)
    elif min_seqlen_array is not None:
        # Generate per-element lengths respecting individual minimums
        low_fn = (
            (lambda m: max(m, max_seqlen - 128))
            if mode == "random"
            else (lambda m: max(m, max_seqlen // 3))
        )
        lengths = torch.tensor(
            [
                [torch.randint(low_fn(int(m)), max_seqlen + 1, (1,)).item()]
                for m in min_seqlen_array
            ],
            device=device,
            dtype=torch.int32,
        )
...
def generate_qkv(
    ...
    if query_padding_mask is not None:
        ...
        rel_bias_unpad = (
            rearrange(rel_bias, "b s ... -> (b s) ...")[indices_q] if rel_bias is not None else None
        )
    else:
        ...
        rel_bias_unpad = (
            rearrange(rel_bias, "b s ... -> (b s) ...") if rel_bias is not None else None
        )
    ...
    if qkvpacked:
        ...
        return (
            ...
            rel_bias_unpad.detach() if rel_bias is not None else None,
            cu_seqlens_q,
            cu_seqlens_k,
            seqused_q,
            seqused_k,
...
def attention_ref(
    ...
    dv = v.shape[-1]
    softmax_scale = (
        1.0 / math.sqrt(d if qv is None else d + dv) if softmax_scale is None else softmax_scale
    )
    if not reorder_ops:
    ...
    if rel_bias is not None:
        ...
        q_idx = torch.arange(seqlen_q, device=q.device, dtype=torch.long)  # [seqlen_q]
        kv_idx = torch.arange(seqlen_k, device=q.device, dtype=torch.long)  # [seqlen_k]
        # [b, seqlen_q, seqlen_k]
        rel_dist = (
            q_idx.unsqueeze(1)
            - kv_idx.unsqueeze(0)  # [seqlen_q, seqlen_k]
            + seqlen_offset.view(-1, 1, 1)  # [b, 1, 1]
        )
        safe_dist = rel_dist.clamp(0, rel_extent - 1)  # [b, seqlen_q, seqlen_k]
        is_within_window = (rel_dist >= 0) & (rel_dist < rel_extent)
        # rel_bias: [b, seqlen_q, h, rel_extent]
        # idx:      [b, seqlen_q, h, seqlen_k]
