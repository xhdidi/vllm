...
def pack_gqa_layout(T, qhead_per_kvhead, nheads_kv, head_idx, seqlen_idx=0):
    """Reshape a tensor to fold qhead_per_kvhead into the seqlen dimension.

    The head dimension is at mode ``head_idx`` and the seqlen dimension at mode
    ``seqlen_idx`` (both may be negative). All other modes are kept as-is.

    For Q/O tensors (seqlen_idx=0, head_idx=2):
        (seqlen_q, headdim, nheads, batch, ...)
            -> ((qhead_per_kvhead, seqlen_q), headdim, nheads_kv, batch, ...)
    For LSE tensors (seqlen_idx=0, head_idx=1):
        (seqlen_q, nheads, batch, ...)
            -> ((qhead_per_kvhead, seqlen_q), nheads_kv, batch, ...)
    For (..., s, n, h) tensors (seqlen_idx=-3, head_idx=-2):
        (..., seqlen, nheads, headdim)
            -> (..., (qhead_per_kvhead, seqlen), nheads_kv, headdim)
    """
    rank = len(T.shape)
    head_idx = head_idx % rank
    seqlen_idx = seqlen_idx % rank
    assert head_idx != seqlen_idx

    head_stride = T.stride[head_idx]

    shape_packed = tuple(
        (qhead_per_kvhead, T.shape[i])
        if i == seqlen_idx
        else nheads_kv
        if i == head_idx
        else T.shape[i]
    ...
    stride_packed = tuple(
        (head_stride, T.stride[i])
        if i == seqlen_idx
        else head_stride * qhead_per_kvhead
        if i == head_idx
        else T.stride[i]
        for i in range(rank)
    )
