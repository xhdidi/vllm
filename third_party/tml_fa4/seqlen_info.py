...
class SeqlenInfoQK:
    ...
    padded_offset_q_unscaled: Int32
    ...
    def offset_batch_Q(
        ...
        rescale_padded_packgqa: cutlass.Constexpr[bool] = False,
    ) -> cute.Tensor:
        padded_offset_q = (
            self.padded_offset_q
            if const_expr(not rescale_padded_packgqa)
            else self.padded_offset_q_unscaled
        )
        ...
        if const_expr(not ragged):
            if const_expr(not self.has_cu_seqlens_q):
                ...
                offset_q = self.offset_q if const_expr(not padded) else padded_offset_q
