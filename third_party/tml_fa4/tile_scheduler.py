...
import vllm.third_party.tml_fa4.utils as utils
from vllm.third_party.tml_fa4.fast_math import clz
...
class StaticPersistentTileScheduler:
    ...
    def get_grid_shape(
        ...
        *,
        ...
        # cute.printf("total_blocks = {}, sm_count = {}, grid_x = {}", params.total_blocks_cluster, sm_count, grid_x)
...
class SingleTileLPTBwdScheduler:
    ...
    class Params(ParamsBase):
        ...
        l2_minor: Int32
        num_head_divmod: FastDivmodDivisor
        num_kvhead_divmod: FastDivmodDivisor
        num_batch_divmod: FastDivmodDivisor
        num_batch_kvhead_divmod: FastDivmodDivisor
        l2_minor_divmod: FastDivmodDivisor
        l2_major_divmod: FastDivmodDivisor
        l2_minor_residual_divmod: FastDivmodDivisor
        num_hb_quotient: Int32
        ...
        def create(
            ...
            *,
            ...
            # If we're in the last section (called residual), we don't want to divide by
            # swizzle. Instead we want to divide by the remainder.
            num_kvhead = args.num_head // args.qhead_per_kvhead
            num_hb_quotient = (args.num_head * args.num_batch) // swizzle
            num_hb_remainder = (args.num_head * args.num_batch) % swizzle
            num_block = cute.ceil_div(args.num_block, args.cluster_shape_mn[0])
            total_blocks = num_block * args.cluster_shape_mn[0] * args.num_batch * args.num_head
            return SingleTileLPTBwdScheduler.Params(
                total_blocks=total_blocks,
                num_block=num_block,
                num_head=args.num_head,
                num_kvhead=num_kvhead,
                qhead_per_kvhead=args.qhead_per_kvhead,
                num_batch=args.num_batch,
                l2_minor=Int32(swizzle),
                num_head_divmod=FastDivmodDivisor(args.num_head),
                num_kvhead_divmod=FastDivmodDivisor(num_kvhead),
                num_batch_divmod=FastDivmodDivisor(args.num_batch),
                num_batch_kvhead_divmod=FastDivmodDivisor(num_kvhead * args.num_batch),
                l2_minor_divmod=FastDivmodDivisor(swizzle),
                ...
                causal=args.causal,
                local=args.local,
                disable_swizzle=args.disable_swizzle and scheduling_mode == SchedulingMode.CLC,
            )
    ...
    def to_underlying_arguments(
        ...
        *,
        ...
        return SingleTileLPTBwdScheduler.Params.create(
            args, scheduling_mode=scheduling_mode, loc=loc, ip=ip
        )
    ...
    def create(
        ...
        tile_idx = cute.arch.block_idx()[0]
        return SingleTileLPTBwdScheduler(params, tile_idx, clc, loc=loc, ip=ip)
    ...
    def get_grid_shape(
        ...
        *,
        ...
        if const_expr(params.disable_swizzle):
            assert params.scheduling_mode == SchedulingMode.CLC, "can only disable swizzle for CLC"
            return (
    ...
    @staticmethod
    def clc_problem_shape(params: Params):
        problem_shape_ntile_mnl = SingleTileLPTBwdScheduler.get_grid_shape(params)
        cluster_shape_mnk = (params.cluster_shape_mn[0], 1, 1)
        return ClcDynamicPersistentTileSchedulerParams(
    ...
    @cute.jit
    def get_current_work(self, *, initial=False, loc=None, ip=None) -> cutlass.utils.WorkTileInfo:
        params = self.params
        if const_expr(self.params.scheduling_mode == SchedulingMode.CLC):
            work = (
                self.clc.get_current_work()
                if const_expr(not initial)
                else self.clc.initial_work_tile_info()
            )
            tile_idx = work.tile_idx[0]
            is_valid = work.is_valid_tile
            if const_expr(params.disable_swizzle):
                block = tile_idx
                head_idx = work.tile_idx[1]
                batch_idx = work.tile_idx[2]
                return WorkTileInfo(
        ...
        cluster_idx = tile_idx // self.params.cluster_shape_mn[0]
        # Implement LPT scheduling coordinate calculation
        ...
        if bidhb < params.num_hb_quotient:
            ...
            block, bidhb_residual = divmod(l2_mod, params.l2_minor_residual_divmod)
        bidhb_actual = bidhb * params.l2_minor + bidhb_residual
        if const_expr(params.scheduling_mode == SchedulingMode.CLC and params.qhead_per_kvhead > 1):
            if const_expr(params.causal):
                head_idx, batch_idx = divmod(bidhb_actual, params.num_batch_divmod)
            else:
                qhead_idx, batch_kvhead_idx = divmod(bidhb_actual, params.num_batch_kvhead_divmod)
                batch_idx, kvhead_idx = divmod(batch_kvhead_idx, params.num_kvhead_divmod)
                head_idx = kvhead_idx * params.qhead_per_kvhead + qhead_idx
        else:
            batch_idx, head_idx = divmod(bidhb_actual, params.num_head_divmod)
    ...
    def initial_work_tile_info(self, *, loc=None, ip=None):
        return self.get_current_work(initial=True, loc=loc, ip=ip)
    ...
    def _varlen_coord_map(self) -> WorkTileInfo:
        ...
        if batch_idx >= params.num_batch:
            ...
            if cutlass.const_expr(params.lpt or params.head_swizzle):
                ...
                if const_expr(params.head_swizzle and params.qhead_per_kvhead > 1):
                    block_div_factor = 32 if const_expr(params.causal) else 16
                    nhead_cap = min(
                        cute.ceil_div(num_n_blocks, block_div_factor) * params.num_kvhead,
                        params.num_head,
                    )
                    nheads_in_l2 = min(nheads_in_l2, nhead_cap)
                else:
                ...
                if const_expr(params.head_swizzle and params.qhead_per_kvhead > 1):
                    # transform head_idx so that kv heads are fastest
                    head_idx, kvhead_idx = divmod(head_idx, params.num_kvhead_divmod)
                    head_idx = kvhead_idx * params.qhead_per_kvhead + head_idx
            else:
...
class DynamicPersistentVarlenScheduler:
    ...
    class Params(ParamsBase):
        ...
        def create(
            ...
            assert args.tile_count_semaphore is not None, "Tile count semaphore must be provided"
    ...
    def initial_work_tile_info(self, *, loc=None, ip=None) -> WorkTileInfo:
        first_tile_idx, _, _ = cute.arch.block_idx()
        return self.get_current_work(first_tile_idx, 0, 0)
    ...
    def prefetch_next_work(
        ...
        *,
        ...
        work_info = self.get_current_work(next_tile_idx, self._bidb_start, self._tile_idx)
        self._scheduler_pipeline.producer_acquire(self._producer_state)
        if cute.arch.lane_idx() == 0:
            ...
            self._scheduler_pipeline.producer_commit(self._producer_state)
        self._producer_state.advance()

    @cute.jit
    def advance_to_next_work(self, *, loc=None, ip=None) -> WorkTileInfo:
        params = self.params
        self._scheduler_pipeline.consumer_wait(self._consumer_state)
        ...
        cute.arch.sync_warp()
        if cute.arch.lane_idx() == 0:
            self._scheduler_pipeline.consumer_release(self._consumer_state)
