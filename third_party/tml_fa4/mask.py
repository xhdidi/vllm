...
class AttentionMask:
    ...
    def apply_mask_sm100_transposed(
        ...
        seqlenk_col_limit = self.seqlen_k - n_block * self.tile_n - thr_col_offset
        seqlen_q = self.seqlen_q * self.qhead_per_kvhead_packgqa
        ...
        if const_expr(not mask_causal and not mask_local and mask_mod is not None):
            ...
            if is_full_block:
                if const_expr(mask_seqlen):
                    if seqlenk_col_limit <= 0:
                        ...
                        for i in cutlass.range_constexpr(ncol):
                            ...
                            q_out_of_bounds = global_q >= seqlen_q
                ...
                for i in cutlass.range_constexpr(ncol):
                    ...
                    if const_expr(mask_seqlen):
                        ...
                        q_out_of_bounds = check_m_boundary and (global_q >= seqlen_q)
                        kv_out_of_bounds = global_kv >= self.seqlen_k
                        out_of_bounds = q_out_of_bounds or kv_out_of_bounds
                        acc_S[i] = -cutlass.Float32.inf if out_of_bounds else acc_S[i]
            ...
            seqlenq_row_limit = seqlen_q - m_block * self.tile_m - thr_row_offset
    ...
    @cute.jit
    def apply_mask_sm100_transposed_seqlen_only(
        self,
        acc_S: cute.Tensor,
        tScS_t2r: cute.Tensor,
        t0ScS_t2r: cute.Tensor,
        ...
        if check_m_boundary:
            # Last m_block: check Q boundary only
            ncol = const_expr(cute.size(tScS_t2r.shape))
            for i in cutlass.range_constexpr(ncol):
                row_coord = tScS_t2r[i][ROW]
                global_q = row_coord + m_block * self.tile_m
                q_out_of_bounds = global_q >= seqlen_q
                acc_S[i] = -cutlass.Float32.inf if q_out_of_bounds else acc_S[i]
